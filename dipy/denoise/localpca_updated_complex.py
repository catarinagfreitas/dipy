# -*- coding: utf-8 -*-
"""
Created on Wed Dec 10 13:16:03 2025

@author: catarinagfreitas
"""
...
def genpca(
    ...
    *,
    ...
    # UPDATED: to be able to deal with complex data
    ...
    # print('patch_radius_arr: ', patch_radius_arr);
    ...
    for k in range(patch_radius_arr[2], arr.shape[2] - patch_radius_arr[2]):
        for j in range(patch_radius_arr[1], arr.shape[1] - patch_radius_arr[1]):
            for i in range(patch_radius_arr[0], arr.shape[0] - patch_radius_arr[0]):
                ...
                if is_svd:
                    #UPDATEDDDD to consider complex matrices
                    # PCA using an SVD
                ...
                ncomps = np.sum(d < tau)
                print('Number of components identified as noise: ', ncomps);
                W[:, :ncomps] = 0
                # print('W ', W)
                
                n_sig = len(eigh(C)[0])-ncomps;
                
                all_sig.append(n_sig)
                variances.append(this_var)
    ...
    # print('denoised_arr ', denoised_arr)
    print('mean number of signal components:', sum(all_sig)/len(all_sig))
    print('min value of signal components identified', min(all_sig))
    print('max value of signal components identified', max(all_sig))
    
    print('mean var:', sum(variances)/len(variances))
    print('min value of var', min(variances))
    print('max value of var', max(variances))
    ...
    np.save("C:/Users/catarinagfreitas/PIC2/Data/"+arr+'_den', denoised_arr)
    print('mean number of signal components:', sum(all_sig)/len(all_sig))
    print('min value of signal components identified', min(all_sig))
    print('max value of signal components identified', max(all_sig))
